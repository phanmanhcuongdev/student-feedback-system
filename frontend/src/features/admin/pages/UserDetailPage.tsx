...
export default function UserDetailPage() {
    ...
    async function handleToggleActive() {
        ...
        try {
            ...
            if (!response.success) {
                setError(response.message || "Unable to update user status.");
                return;
            }
            ...
            <div className="mx-auto max-w-screen-lg px-6 py-10">
                    ...
                    {loading ? (
                        ...
                        <div className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
                            <section className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-[0_18px_40px_rgba(15,23,42,0.05)]">
                                ...
                                <div className="mt-6 grid gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">
                                    ...
                                    <div className="flex items-center justify-between gap-4">
                                        ...
                                        <span className="font-medium text-slate-900">{user.departmentName ?? "N/A"}</span>
                                    </div>
                                    <div className="flex items-center justify-between gap-4">
                                ...
                                >
                                    <span>{toggling ? "Updating..." : user.active ? "Deactivate user" : "Activate user"}</span>
                            ...
                            <section className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-[0_18px_40px_rgba(15,23,42,0.05)]">
                                <h2 className="text-2xl font-bold text-slate-950">Edit basic info</h2>
                                <p className="mt-2 text-sm text-slate-500">
                                    Update core user information without changing role or survey permissions.
                                </p>
                                ...
                                <form className="mt-6 space-y-5" onSubmit={handleSave}>
                                    ...
                                    {user.role !== "ADMIN" ? (
                                        <label className="block space-y-2">
                                            <span className="text-sm font-semibold text-slate-700">Department ID</span>
                                            <input
                                                type="number"
                                                value={departmentId}
