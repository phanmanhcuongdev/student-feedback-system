...
**Lớp/Học phần:** Nhập môn Công nghệ phần mềm
**Thời gian thực hiện:** Hà Nội, 2026

| Thành viên | Mã sinh viên |
|---|---|
| Phan Mạnh Cường | B23DCCN115 |
| Nguyễn Hữu Tâm | B23DCCN736 |
| Đỗ Xuân Sang | B23DCCN709 |
...
Nhóm thực hiện xin gửi lời cảm ơn đến cô Nguyễn Thị Thanh Thủy và nhà trường đã tạo điều kiện để nhóm nghiên cứu, thiết kế và triển khai đề tài “Hệ thống khảo sát ý kiến sinh viên”. Trong quá trình thực hiện, nhóm có cơ hội vận dụng các kiến thức đã học về phân tích yêu cầu, thiết kế hệ thống, lập trình web, cơ sở dữ liệu, bảo mật, triển khai ứng dụng và vận hành hạ tầng.
...
Phạm vi phân tích trong báo cáo dựa trên mã nguồn, cấu hình thực tế của dự án và thông tin hạ tầng vận hành do nhóm triển khai. Quy trình CI/CD trong mã nguồn hiện dừng ở kiểm thử, build, scan bảo mật và push Docker image; bước triển khai tự động lên server production chưa được đưa vào workflow GitHub Actions, nên báo cáo trình bày phần triển khai runtime như một hoạt động vận hành tách khỏi pipeline tự động.

## 4. Phương pháp thực hiện

Báo cáo được xây dựng theo phương pháp kết hợp giữa phân tích mã nguồn và phân tích kiến trúc hệ thống:

- Đọc cấu trúc mã nguồn để xác định module backend, frontend, database, deployment, CI/CD và tài liệu.
...
- Đối chiếu với thông tin hạ tầng triển khai của nhóm như Proxmox VE, VyOS, VLAN, Headscale/Tailscale, VPS nginx-proxy, Prometheus, Grafana và Loki.

Với các nội dung chưa có đủ bằng chứng trong mã nguồn hoặc tài liệu triển khai, nhóm ghi rõ phạm vi nhận xét và không khẳng định vượt quá dữ liệu đã kiểm chứng.
...
| Availability | Ứng dụng được container hóa, có thể chạy frontend/backend độc lập; hạ tầng triển khai có reverse proxy và monitoring để hỗ trợ vận hành. |
...
| Observability | Theo thông tin triển khai, monitoring stack gồm Prometheus, Grafana OSS và Loki; cấu hình monitoring được vận hành ở stack riêng ngoài mã nguồn ứng dụng chính. |
...
![Use Case Diagram tổng quát](assets/diagrams/use-case-overview.png)
...
Qua quá trình khảo sát mã nguồn và giao diện, hệ thống có các actor chính sau:
...
Sơ đồ được rút gọn có chủ đích để phù hợp với báo cáo học thuật: các chức năng nội bộ như ghi audit log, tạo snapshot người nhận khi publish survey, lưu tài liệu qua MinIO, gửi email qua Resend, xử lý RabbitMQ hoặc gọi AI/embedding service đã được phân tích ở phần kiến trúc và triển khai, nhưng không tách thành use case riêng trên sơ đồ tổng quát để tránh làm hình quá rối. Cách trình bày này giúp người đọc nhanh chóng nắm được phạm vi nghiệp vụ chính của hệ thống, đồng thời vẫn nhất quán với mã nguồn backend, frontend và migration đã khảo sát.
...
![Activity Diagram - Quy trình onboarding sinh viên](assets/diagrams/activity-student-onboarding.png)
...
![Activity Diagram - Quy trình tạo và publish khảo sát](assets/diagrams/activity-survey-publish.png)
...
![Activity Diagram - Quy trình sinh viên nộp khảo sát](assets/diagrams/activity-submit-survey.png)
...
![Sequence Diagram - Đăng nhập bằng JWT](assets/diagrams/sequence-login-jwt.png)
...
![Sequence Diagram - Nộp khảo sát](assets/diagrams/sequence-submit-survey.png)
...
![Sequence Diagram - Generate AI Summary](assets/diagrams/sequence-ai-summary.png)
...
![Sequence Diagram - Realtime Notification](assets/diagrams/sequence-notification.png)
...
![Domain Model tổng quát](assets/diagrams/domain-model-overview.png)
...
### 2.7 Các nghiệp vụ hỗ trợ khai thác dữ liệu

Ngoài luồng khảo sát chính, hệ thống còn có một số nghiệp vụ hỗ trợ giúp dữ liệu phản hồi có giá trị sử dụng cao hơn trong thực tế vận hành.

**AI Summary cho phản hồi dạng văn bản:** các phản hồi tự luận thường chứa nhiều thông tin định tính nhưng khó tổng hợp thủ công khi số lượng comment lớn. Chức năng AI Summary giúp Admin hoặc Lecturer có quyền nhanh chóng nắm được các ý chính, vấn đề nổi bật, điểm tích cực, điểm cần cải thiện và gợi ý hành động. Tuy nhiên, summary chỉ đóng vai trò hỗ trợ phân tích; dữ liệu phản hồi gốc vẫn được lưu trữ đầy đủ và là nguồn đối chiếu chính khi cần kiểm chứng.

Trong mã nguồn backend, nhóm chức năng này được thể hiện qua `SurveyAiSummaryController`, `SurveyAiSummaryService`, các bảng `Survey_AI_Summary`, `Survey_AI_Summary_Job`, `Survey_AI_Source_State`, `Survey_AI_Pending_Change` và `Survey_AI_Summary_Theme_Embedding`. Thiết kế này cho phép hệ thống lưu trạng thái xử lý, theo dõi dữ liệu nguồn, tái sử dụng kết quả đã có khi dữ liệu chưa thay đổi đáng kể và xử lý lại khi dữ liệu comment thay đổi. Cách tiếp cận này giúp giảm số lần gọi AI provider, tiết kiệm tài nguyên và tránh tạo nhiều bản summary trùng lặp cho cùng một tập dữ liệu.

**Xuất báo cáo khảo sát:** kết quả khảo sát không chỉ được xem trực tiếp trên dashboard mà còn cần xuất ra file phục vụ lưu trữ, chia sẻ hoặc nộp báo cáo nội bộ. Backend có `SurveyReportExportService`, `ExportSurveyReportUseCase`, `SurveyReportRenderer` và adapter BIRT để render báo cáo PDF/XLSX. Về nghiệp vụ, báo cáo có thể bao gồm thông tin chung của survey, tỷ lệ phản hồi, thống kê theo câu hỏi rating, phản hồi text và AI Summary nếu có. Việc tách phần tổng hợp dữ liệu khỏi phần render file giúp chức năng báo cáo dễ bảo trì và có thể thay đổi template hoặc định dạng xuất trong tương lai.

**Notification và reminder:** notification giúp hệ thống truyền tải các sự kiện quan trọng như khảo sát mới, trạng thái onboarding hoặc phản hồi feedback. Mỗi notification được lưu trong database và gắn với người nhận thông qua `Notification_User`, nhờ đó người dùng vẫn xem được thông báo sau khi đăng nhập lại. Hệ thống cũng có WebSocket/STOMP để đẩy thông báo realtime đến frontend khi người dùng đang online. Đối với reminder khảo sát, backend có scheduled task và các port liên quan đến survey reminder candidate, giúp nhắc sinh viên chưa nộp khi survey còn mở và gần hết hạn.

**Dịch/xử lý text bất đồng bộ:** RabbitMQ trong hệ thống được sử dụng chủ yếu cho các tác vụ translation liên quan đến dữ liệu text, thể hiện qua `RabbitMQTranslationAdapter`, `TranslationReplyListener` và `RabbitMqConfig`. Backend Spring Boot đóng vai trò producer khi phát sinh task dịch cho survey, question, feedback hoặc câu trả lời tự luận; đồng thời đóng vai trò consumer ở reply queue để nhận kết quả dịch và cập nhật lại các cột song ngữ trong SQL Server. Worker xử lý dịch nằm ở repository riêng `E:\Lap\TTCS\ai-translation-wrapper`. Đây là một FastAPI service consume queue từ RabbitMQ, gọi Ollama chạy model `qwen2:7b`, lưu cache dịch song ngữ trong PostgreSQL, sau đó publish kết quả về reply queue. Vì vậy, báo cáo mô tả RabbitMQ như một thành phần hỗ trợ translation/text processing bất đồng bộ, không mở rộng thành mọi tác vụ nền nếu chưa có bằng chứng trong mã nguồn.
...
- **Integration Layer:** các tích hợp ngoài gồm RabbitMQ cho task dịch bất đồng bộ, FastAPI translation worker ngoài repo dùng Ollama `qwen2:7b`, MinIO cho lưu trữ tài liệu sinh viên, Resend cho email, AI/Gemini-compatible provider cho tóm tắt phản hồi, embedding service cho vector hóa nội dung text và Eclipse BIRT cho xuất báo cáo PDF/XLSX.
...
![System Architecture Diagram tổng thể](assets/diagrams/system-architecture.png)
...
### 3.2 Tổ chức mã nguồn dự án

Mã nguồn dự án được tổ chức thành các module rõ ràng cho backend, frontend, database, embedding service, tài liệu và cấu hình triển khai. Bảng sau mô tả vai trò của các thành phần chính:
...
| Embedding Service | `embedding-service/` | Thành phần phụ trợ FastAPI phục vụ tạo embedding cho nội dung text, dùng model `intfloat/multilingual-e5-small` theo thông tin trong repo. |
| Translation Worker | `E:\Lap\TTCS\ai-translation-wrapper` | FastAPI service consume RabbitMQ translation task, gọi Ollama với model `qwen2:7b`, cache kết quả trong PostgreSQL và publish kết quả dịch về reply queue để backend cập nhật dữ liệu song ngữ. |
...
| README | `README.md` | Tài liệu giới thiệu và hướng dẫn tổng quan của dự án. |
...
![Backend Hexagonal Architecture Diagram](assets/diagrams/backend-hexagonal-architecture.png)
...
- **`infrastructure`:** chứa cấu hình hạ tầng nội bộ như WebSocket/STOMP và scheduling. Trong mã nguồn có các package `infrastructure/configuration` và `infrastructure/scheduling`.
...
- **Dễ bảo trì lâu dài:** package được chia theo hướng inbound/outbound/application core giúp giảm phụ thuộc vòng và giúp nhóm phát triển dễ xác định vị trí sửa đổi.
...
Về nguyên tắc thiết kế, API được tổ chức theo domain nghiệp vụ thay vì gom toàn bộ vào một controller lớn. Các API public như đăng nhập, đăng ký, xác minh email và quên/reset mật khẩu không yêu cầu JWT nhưng vẫn cần validation dữ liệu đầu vào. Các API protected yêu cầu token hợp lệ trong header `Authorization: Bearer ...`; backend xác định người dùng hiện tại từ token thay vì để frontend tự gửi `userId` cho các thao tác cá nhân. Các API quản trị được tách dưới nhóm `/api/admin/**`, giúp rule phân quyền ở `SecurityConfig` rõ hơn và dễ kiểm soát hơn.

Đối với các danh sách có khả năng tăng kích thước như user, survey, feedback, notification, audit log hoặc survey result, backend và frontend cần duy trì cách xử lý theo hướng phân trang, tìm kiếm, lọc và sắp xếp. Cách thiết kế này giúp giảm dữ liệu truyền tải, tránh tải toàn bộ bảng lên client và phù hợp với yêu cầu vận hành khi số lượng sinh viên hoặc số lượng khảo sát tăng lên.
...
![Frontend Architecture Diagram](assets/diagrams/frontend-architecture.png)
...
Về xử lý bất đồng bộ, mã nguồn hiện thể hiện ba nhóm đáng chú ý. Thứ nhất, RabbitMQ được dùng cho translation task/reply để xử lý nội dung text mà không làm phức tạp request chính. Thứ hai, AI Summary có các adapter async và job/source-state để theo dõi quá trình tạo summary, cập nhật khi dữ liệu nguồn thay đổi và tránh gọi lại AI provider không cần thiết. Thứ ba, survey reminder được xử lý bằng scheduled task ở tầng infrastructure, giúp hệ thống có thể nhắc sinh viên chưa nộp khảo sát mà không phụ thuộc vào thao tác thủ công của Admin. Các cơ chế này tăng khả năng vận hành nhưng vẫn nằm quanh backend chính, không biến toàn bộ hệ thống thành microservice thuần túy.
...
- **Có CI/CD và artifact triển khai:** dự án có GitHub Actions, Dockerfile backend/frontend, `docker-compose.yml`, Nginx config và `.env.example`.
- **Có nền tảng vận hành/monitoring/hardening:** theo mô hình triển khai của nhóm, hệ thống có reverse proxy, VPN management, phân tách mạng, Prometheus/Grafana/Loki, fail2ban và CrowdSec.

Các điểm cần cải thiện hoặc cần lưu ý:

- **`docker-compose.yml` chủ yếu đóng gói frontend/backend:** các thành phần như SQL Server, RabbitMQ, MinIO, AI provider và monitoring được vận hành ở hạ tầng ngoài hoặc stack riêng. Khi bàn giao hoặc tái triển khai, cần có tài liệu môi trường rõ hơn.
- **Cần kiểm tra WebSocket proxy trong môi trường production:** nếu realtime notification dùng trong production, reverse proxy phải hỗ trợ upgrade WebSocket và timeout phù hợp.
- **Cần bổ sung application-level metrics:** monitoring hiện chủ yếu bao phủ tầng host/container. Nếu muốn observability sâu hơn, backend nên có metrics nghiệp vụ/kỹ thuật như request latency, survey submit count, queue lag, AI job duration và error rate.
...
Trong dự án cũng có thư mục `database/` với các file như `full_schema.sql`, `seed_data.sql` và `database/migrations/`. Theo `database/README.md`, các file này dùng cho thiết lập thủ công, dữ liệu demo hoặc tham khảo/lịch sử trước khi chuyển sang Flyway. Vì vậy, khi phân tích schema chính thức của ứng dụng, báo cáo ưu tiên Flyway migration trong backend.

### 5.4 Embedding service, translation worker và AI integration

Dự án có module `embedding-service/`, là một service FastAPI độc lập phục vụ tạo embedding cho nội dung text. File `embedding-service/app/main.py` khai báo model:
...
Phần dịch song ngữ được tách khỏi request chính bằng RabbitMQ. Trong repo backend, `RabbitMQTranslationAdapter` publish message gồm `entity_id`, `entity_type`, `content`, `source_lang` và `target_lang`; `TranslationReplyListener` lắng nghe reply queue và chuyển kết quả vào `TranslationResultService`. Worker thực hiện dịch nằm ở repository `E:\Lap\TTCS\ai-translation-wrapper`. Đây là FastAPI service khởi động RabbitMQ consumer trong lifespan của ứng dụng, đồng thời expose `GET /health` để kiểm tra PostgreSQL, RabbitMQ, queue, DLQ, model và cấu hình xử lý.

Theo contract của worker, request message gồm `entity_id`, `entity_type`, `content` và `source_lang`; các `entity_type` hợp lệ là `FEEDBACK`, `QUESTION`, `SURVEY_DESCRIPTION`, `SURVEY_RESPONSE` và `SURVEY_TITLE`. Worker dùng Ollama qua `ollama.AsyncClient`, model mặc định `qwen2:7b`, `OLLAMA_NUM_CTX=2048`, timeout 180 giây và yêu cầu model trả JSON có đúng ba trường `source_lang`, `translated_content_vi`, `translated_content_en`. Kết quả reply được publish về `translation.reply.exchange` với routing key `translation.reply.key`, gồm `translated_content_vi`, `translated_content_en`, `source_lang`, `model_info` và `is_auto_translated`.

Worker có PostgreSQL riêng để cache kết quả dịch trong bảng `translation_cache`, định danh bằng `hash_key`, `model_info` và `target_lang='multi'`. Khi cache hit, worker trả kết quả ngay mà không gọi Ollama. Khi cache miss, worker dùng PostgreSQL advisory lock với key cấu hình `GPU_ADVISORY_LOCK_KEY=1050` để chỉ cho một task GPU chạy tại một thời điểm, phù hợp với môi trường GPU tài nguyên hạn chế. Worker cũng có cơ chế retry thông qua header `x-retry-count`, `RABBITMQ_MAX_RETRIES=3` và dead-letter queue `translation.dead-letter.queue` cho message lỗi hoặc vượt quá số lần thử. Cách làm này giúp thao tác tạo khảo sát, gửi feedback hoặc nộp khảo sát không phải chờ tác vụ dịch hoàn tất, đồng thời tránh gọi lại model cho nội dung đã dịch trước đó.

Ngoài embedding local và worker dịch, backend còn có `ChatCompletionSurveyCommentSummaryAdapter` để gọi AI/Gemini-compatible API thông qua cấu hình `app.ai.base-url`, `app.ai.api-key` và `app.ai.model`. File `.env.example` đang cấu hình mẫu theo OpenAI-compatible endpoint của Google Generative Language API và model `gemini-2.5-flash-lite`. Luồng này được dùng cho chức năng summary/phân tích phản hồi text của khảo sát. Trong phạm vi code hiện tại, không nên mô tả AI như một hệ thống chấm điểm toàn diện; chức năng chính được thể hiện rõ là tạo summary có cấu trúc, phân tích highlights/concerns/actions và hỗ trợ embedding cho nội dung text.
...
![Containerization Diagram](assets/diagrams/containerization-diagram.png)
...
### 5.6 Docker Compose deployment trong dự án
...
Compose trong dự án hiện đóng gói hai thành phần ứng dụng chính là frontend và backend. Các service như SQL Server, RabbitMQ, MinIO, AI provider, embedding service và monitoring stack không nằm trong compose này. Các thành phần đó được vận hành ở hạ tầng ngoài, stack riêng hoặc cấu hình runtime khác theo mô hình triển khai của nhóm.
...
![CI/CD Pipeline Diagram](assets/diagrams/cicd-pipeline.png)

> [Hình 5.2: CI/CD Pipeline Diagram gồm quality gate, image build, image scan và push GHCR]

### 5.8 Giới hạn của pipeline hiện tại

Pipeline hiện có CI tương đối đầy đủ cho build/test/lint/security scan và tạo Docker image. Tuy nhiên, trong `.github/workflows/ci.yml` chưa có bước CD tự động deploy trực tiếp lên server bằng SSH, Kubernetes, Docker Compose remote hoặc công cụ tương đương.

Vì vậy, có thể kết luận về phạm vi CI/CD hiện tại:

- Có CI, security scan, build Docker image và push GHCR.
- Chưa có CD tự động deploy lên server trong workflow.
- Việc deploy runtime có thể được thực hiện thủ công, qua Portainer hoặc qua compose stack riêng trong hạ tầng vận hành.
...
- **Có Docker image:** backend và frontend được build thành image và push lên GHCR khi push code.
...
- **Có nền tảng bảo trì lâu dài:** Maven, Vite, Docker, Flyway và GitHub Actions tạo nền tảng ổn định cho kiểm thử, build và vận hành.
...
Chương 5 cho thấy dự án đã có nền tảng công nghệ và artifact triển khai tương đối rõ: backend/frontend được build, kiểm thử, scan và đóng gói image. Tuy nhiên, triển khai thực tế không chỉ phụ thuộc vào mã nguồn mà còn cần VPS, VPN, Proxmox, VLAN, reverse proxy và monitoring. Vì vậy, Chương 6 sẽ tiếp tục trình bày kiến trúc hạ tầng mạng và mô hình triển khai vận hành của hệ thống.
...
Theo mô hình hạ tầng của nhóm, môi trường triển khai sử dụng Proxmox VE làm nền tảng ảo hóa, VyOS Router VM làm gateway Layer 3, Cisco Switch làm Layer 2 switch/trunk VLAN, Headscale/Tailscale VPN Mesh cho quản trị, VPS public làm điểm ingress qua Nginx reverse proxy và một node riêng cho monitoring. Cách tổ chức này giúp hệ thống khảo sát có mô hình triển khai gần với thực tế vận hành: public traffic được kiểm soát qua reverse proxy, còn các cổng quản trị và dịch vụ nội bộ không bị đưa trực tiếp ra Internet.

Nếu triển khai toàn bộ thành phần trên một mạng phẳng, rủi ro vận hành và bảo mật sẽ tăng đáng kể. Khi một service bị khai thác, attacker có thể scan cùng subnet để dò SSH, database, RabbitMQ, MinIO hoặc giao diện quản trị. Mạng phẳng cũng làm mờ ranh giới giữa traffic người dùng, traffic quản trị, traffic monitoring và traffic nội bộ giữa backend với database/queue/storage. Vì vậy, thiết kế hạ tầng của nhóm ưu tiên phân vùng mạng ngay từ đầu bằng VLAN, route tập trung qua VyOS và quản trị qua VPN Mesh thay vì mở port trực tiếp ra Internet.
...
- **Không phụ thuộc router ISP cho định tuyến nội bộ:** router ISP chỉ đóng vai trò uplink Internet; định tuyến liên VLAN, NAT nội bộ và route về Tailnet được đặt ở VyOS để dễ kiểm soát, backup và tái tạo cấu hình.
- **Ưu tiên khả năng kiểm chứng:** thiết kế hạ tầng cần có bằng chứng vận hành như Headscale node/tag, Portainer/Proxmox screenshot, public port check, monitoring target và sơ đồ topology, thay vì chỉ mô tả ở mức lý thuyết.

### 6.3 Kiến trúc mạng tổng thể

![Network Topology tổng thể](assets/diagrams/network-topology.png)
...
Ở tầng ảo hóa, Proxmox được thiết kế với hai hướng kết nối chính. `vmbr0` phục vụ uplink/WAN cho VyOS đi ra phía ISP, còn `vmbr1` là bridge nội bộ VLAN-aware, nối ra switch vật lý và cho phép nhiều VLAN đi qua cùng một trunk. VyOS nhận trunk nội bộ qua một interface và tạo các sub-interface như `eth1.10`, `eth1.20`, `eth1.99`; mô hình này tương đương router-on-a-stick trong môi trường ảo hóa. Cisco Switch xử lý trunk 802.1Q ở Layer 2, còn VyOS chịu trách nhiệm Layer 3, giúp vai trò switching và routing được phân định rõ ràng.

### 6.4 Public access flow

![Public Access Flow](assets/diagrams/public-access-flow.png)
...
![Management Access Flow qua Tailnet](assets/diagrams/management-access-flow.png)
...
Headscale control plane sử dụng dải Tailnet `100.64.0.0/10` và gắn tag theo vai trò cho từng node. Các tag quan trọng gồm `sysadmin`, `core-router`, `nginx-proxy`, `hypervisor`, `monitoring` và `control-plane`. Cách định danh theo tag giúp chính sách truy cập không phụ thuộc cứng vào từng IP, mà bám vào vai trò vận hành của node.

Trong thiết kế route, `bgp-speaker/core-router` là điểm nối giữa VLAN 99 và Tailnet. Node này có IP nội bộ `10.1.99.10` và Tailnet IP `100.64.0.2`, giúp các subnet nội bộ có thể được tiếp cận qua một điểm trung gian được kiểm soát. Cách làm này tốt hơn việc cài VPN client trên mọi VM ứng dụng, vì route về mạng nội bộ được tập trung và dễ quản lý hơn.
...
Về mặt địa chỉ mạng, thiết kế sử dụng VLAN 10 cho Production, VLAN 20 cho Lab/Internal và VLAN 99 cho Management. Trong tài liệu hạ tầng của nhóm, VLAN 10 gắn với subnet `192.168.10.0/24`, VLAN 20 gắn với `10.1.20.0/24`, còn VLAN 99 gắn với `10.1.99.0/24`. Các host alias này cũng là cơ sở để viết ACL hoặc firewall rule rõ ràng hơn trong các bước hardening tiếp theo.

VLAN chỉ là ranh giới Layer 2; để đạt mức kiểm soát tốt hơn, traffic liên VLAN cần được đưa qua VyOS để có thể bổ sung firewall policy theo zone trong tương lai. Ví dụ, Lab không nên mặc định truy cập Production, Production không nên chủ động truy cập Management, còn Monitoring chỉ nên được mở các port metrics/SNMP cần thiết.

### 6.8 Security hardening ở tầng hạ tầng

Các biện pháp hardening hạ tầng đã triển khai gồm:
...
Một số rule ACL quan trọng trong thiết kế gồm: `nginx-proxy` chỉ cần truy cập web production qua port 80/443; `monitoring` chỉ cần truy cập các port metrics như 9100, 8080, 9080 và SNMP 161 trên switch; `sysadmin` có quyền rộng hơn để vận hành hạ tầng; còn các vai trò phát triển hoặc service khác không nên được cấp quyền toàn mạng. Nguyên tắc này giúp tránh tình trạng "vào VPN là thấy toàn bộ hệ thống".

Một điểm thực tế đáng chú ý là khi dùng OWASP ZAP để probing trong môi trường kiểm thử, CrowdSec đã phát hiện và ban IP. Điều này cho thấy lớp phát hiện probing ở gateway/VPS có hoạt động; bằng chứng minh họa được trình bày ở Chương 8 và phần demo bảo mật.
...
- **Chưa có dự phòng uplink/trunk vật lý:** nếu đường trunk giữa Proxmox và Cisco gặp lỗi, nhiều VLAN nội bộ có thể bị ảnh hưởng; hướng nâng cấp là LACP hoặc uplink dự phòng.
- **VyOS và Proxmox hiện là các điểm phụ thuộc lớn:** với phạm vi homelab/đồ án có thể chấp nhận được, nhưng môi trường production thật nên cân nhắc HA cho router, ví dụ VRRP hoặc cặp VyOS active/standby.
- **Firewall liên VLAN cần được cụ thể hóa:** hệ thống đã có phân vùng VLAN và route tập trung, nhưng để đạt mức enterprise hơn cần có zone-based policy rõ giữa Production, Lab, Management, VPN và WAN.
...
![Monitoring Plane Overview](assets/diagrams/monitoring-plane-overview.png)

> [Hình 7.1: Monitoring Plane Overview gồm Prometheus, Grafana, Loki, Tailnet targets và sysadmin access]
...
Trạng thái triển khai thực tế cho thấy monitoring stack không chỉ tồn tại ở mức cấu hình. Các container Prometheus, Grafana OSS và Loki đang chạy bằng Docker Compose trong `/opt/monitoring`. Việc đặt stack trên Vostro Hub giúp tách vai trò quan sát khỏi backend/database và các workload Production; nếu một service nghiệp vụ gặp sự cố, monitoring vẫn có cơ hội duy trì độc lập để hỗ trợ điều tra.
...
Theo kết quả kiểm tra vận hành, truy vấn PromQL `up` trả về `1` cho toàn bộ target hiện có. Điều này cho thấy Prometheus đang scrape thành công các target đã cấu hình tại thời điểm kiểm tra.

Các target hiện tại bao phủ các điểm hạ tầng trọng yếu: Proxmox host, container metrics trên Proxmox, VPS nginx-proxy public entrypoint và chính monitoring node. Báo cáo không khẳng định đã có đầy đủ application-level metrics cho backend, database, RabbitMQ, MinIO hoặc AI worker; các thành phần này được xếp vào hướng nâng cấp để tránh mô tả vượt quá bằng chứng hiện có.

![Prometheus Targets Up](assets/screenshots/monitoring/prometheus-targets-up.png)

> [Hình 7.2: Prometheus Targets Up, thể hiện các target hiện có đều ở trạng thái `UP`]
...
Thiết kế này giúp hạn chế exposure, vì dashboard monitoring chỉ truy cập qua Tailnet thay vì mở public. Grafana đã có dashboard hiển thị các chỉ số:
...
Trong dashboard hiện tại, các instance chính được quan sát gồm Cloud VPS `100.64.0.4:9100`, Proxmox `100.64.0.5:9100` và Vostro Hub `100.64.0.6:9100`. Với Cloud VPS, các chỉ số CPU/RAM/network giúp theo dõi public entrypoint cho domain `survey.cuongdso.id.vn`. Với Proxmox, dashboard giúp quan sát hypervisor chạy VM/LXC. Với Vostro Hub, dashboard giúp monitoring node tự giám sát chính nó.

![Grafana Dashboard - Node Exporter](assets/screenshots/monitoring/grafana-node-exporter-dashboard.png)

> [Hình 7.3: Grafana dashboard CPU/RAM/Disk/Network/Uptime]

Ngoài dashboard, hệ thống có cấu hình alert/contact point Telegram để phục vụ cảnh báo vận hành.

Alerting được cấu hình trong Grafana UI và gửi thông báo về Telegram khi điều kiện cảnh báo thỏa mãn. Trong báo cáo chỉ dùng ảnh chụp contact point/thông báo để chứng minh cơ chế đã triển khai; các thông tin nhạy cảm như bot token, chat ID, tài khoản Grafana hoặc mật khẩu quản trị không được đưa vào nội dung báo cáo.

![Grafana Telegram Alert Contact Point](assets/screenshots/monitoring/grafana-telegram-alert.png)

> [Hình 7.4: Grafana Telegram alert contact point]
...
Trong triển khai hiện tại, Loki là thành phần đã chạy trong stack Docker Compose tại `/opt/monitoring`. Ở mức báo cáo này, nhóm chỉ khẳng định Loki đã được triển khai làm nền tảng quan sát log; chi tiết pipeline thu log đầy đủ cho từng service cần được hoàn thiện thêm ở giai đoạn sau.

Cấu hình Loki hiện dùng filesystem backend, schema TSDB v13 và replication factor 1, phù hợp với quy mô nhỏ/lab. Loki cũng có cấu hình loại bỏ log quá cũ theo ngưỡng thời gian, giúp hạn chế việc ingest dữ liệu không còn phù hợp. Đây là nền tảng ban đầu cho log observation, chưa phải một hệ thống log HA hoặc lưu trữ dài hạn.

Do đó, báo cáo không khẳng định hệ thống đã thu toàn bộ log của mọi service. Phần cần tiếp tục hoàn thiện là promtail, log agent hoặc Docker log collector trên từng host/service.
...
![Loki Logs Panel](assets/screenshots/monitoring/loki-logs-panel.png)

> [Hình 7.5: Loki Logs Panel trên Grafana, thể hiện log observation cho một số nguồn log hạ tầng]
...
- **Grafana secret cần quản lý tốt hơn:** tài khoản quản trị, mật khẩu ban đầu, Telegram bot token và chat ID không nên nằm trong tài liệu công khai hoặc file cấu hình không được bảo vệ; về lâu dài cần secret management hoặc provisioning an toàn hơn.
...
- **Chưa có đầy đủ exporter cho mọi service phụ trợ:** cần bổ sung exporter cho SQL Server, RabbitMQ, MinIO và Nginx nếu muốn quan sát sâu hơn.
...
![Security Layers Diagram](assets/diagrams/security-layers.png)

> [Hình 8.1: Security Layers Diagram gồm application security, frontend access control, CI/CD security gate, infrastructure hardening và monitoring/security response]
...
![JWT Authentication Flow](assets/diagrams/jwt-auth-flow.png)

> [Hình 8.2: JWT Authentication Flow từ login, lưu token, gửi Bearer token đến JwtAuthenticationFilter và SecurityContext]
...
Các nhóm hành động được ghi nhận trong mã nguồn gồm:
...
Dự án có GitHub Actions chạy Trivy ở hai lớp:

- **Trivy filesystem scan:** quét toàn bộ mã nguồn với scanner `vuln,secret,misconfig`, severity `CRITICAL`, `exit-code: 1`.
...
1. Ban đầu Trivy phát hiện 5 lỗ hổng mức `CRITICAL` trong dependency backend, đồng thời image scan cũng giúp kiểm tra rủi ro ở runtime image frontend.
2. Các lỗ hổng backend liên quan đến nhóm Spring Boot/Spring Security/Tomcat embedded.
3. Nhóm xử lý bằng cách nâng Spring Boot parent từ `4.0.3` lên `4.0.6`, để BOM của Spring Boot quản lý dependency đã được vá.
4. Với frontend image, nhóm cập nhật package trong runtime image Nginx Alpine để xử lý lỗ hổng ở tầng image.
5. Sau khi chạy lại pipeline, Trivy filesystem scan trả về `0 vulnerabilities` cho backend, frontend và embedding-service; Trivy image scan cũng trả về `0 vulnerabilities` cho các image được kiểm tra, đồng thời Dockerfile không có misconfiguration mức chặn pipeline.

Ý nghĩa của trường hợp này là security gate trong CI/CD có tác dụng thực tế: lỗ hổng được phát hiện trước khi image được push/deploy, buộc nhóm phải nâng dependency thay vì bỏ qua cảnh báo. Đây không chỉ là một bước hình thức trong pipeline, mà là cơ chế chặn rủi ro trước khi artifact đi vào môi trường vận hành.

![GitHub Actions Trivy Scan Passed](assets/screenshots/security/trivy-scan-passed.png)

> [Hình 8.3: GitHub Actions Trivy scan passed, thể hiện filesystem/image scan sau khi xử lý vulnerabilities]
...
![OWASP ZAP Scan Evidence](assets/screenshots/security/zap-scan.png)

> [Hình 8.4: OWASP ZAP scan trong môi trường kiểm soát]

![CrowdSec Decision Ban IP](assets/screenshots/security/crowdsec-ban-ip.png)

> [Hình 8.5: CrowdSec decision ban IP do probing/scan]
...
Các test case chức năng được xây dựng theo từng module nghiệp vụ như sau. Một số luồng đã có backend automated tests; với kiểm thử thủ công/demo, nhóm ghi nhận trạng thái theo hướng cần bổ sung bảng kết quả chạy chi tiết nếu giảng viên yêu cầu.

| Mã test | Chức năng | Điều kiện | Kết quả mong đợi | Trạng thái |
|---|---|---|---|---|
| TC-AUTH-01 | Authentication | Người dùng nhập email/password hợp lệ. | Đăng nhập thành công, nhận JWT và điều hướng theo role. | Cần ghi nhận kết quả chạy chi tiết |
| TC-AUTH-02 | Authentication | Người dùng nhập sai thông tin đăng nhập. | Backend trả lỗi xác thực, không cấp token. | Cần ghi nhận kết quả chạy chi tiết |
| TC-ONB-01 | Student onboarding | Student đăng ký và xác minh email. | Tài khoản chuyển sang trạng thái phù hợp để upload giấy tờ. | Cần ghi nhận kết quả chạy chi tiết |
| TC-ONB-02 | Admin student approval | Admin duyệt student pending. | Student chuyển `ACTIVE`, có notification và audit log. | Cần ghi nhận kết quả chạy chi tiết |
| TC-USER-01 | User management | Admin cập nhật hoặc activate/deactivate user. | Thông tin user thay đổi đúng, thao tác được audit. | Cần ghi nhận kết quả chạy chi tiết |
| TC-SUR-01 | Survey management | Admin tạo survey với câu hỏi hợp lệ. | Survey được tạo ở trạng thái `DRAFT`. | Cần ghi nhận kết quả chạy chi tiết |
| TC-SUR-02 | Survey publish | Admin publish survey hợp lệ. | Survey chuyển trạng thái published, tạo `Survey_Recipient`. | Cần ghi nhận kết quả chạy chi tiết |
| TC-SUB-01 | Student survey submission | Student `ACTIVE`, verified, được gán survey. | Nộp survey thành công, lưu response/detail, cập nhật submitted state. | Cần ghi nhận kết quả chạy chi tiết |
| TC-SUB-02 | Student survey submission | Student đã nộp survey trước đó. | Backend từ chối nộp lại. | Cần ghi nhận kết quả chạy chi tiết |
| TC-FB-01 | Feedback | Student gửi feedback hợp lệ. | Feedback được lưu và staff có thể xem. | Cần ghi nhận kết quả chạy chi tiết |
| TC-NOTI-01 | Notification | User có notification chưa đọc. | Unread count đúng, mark read hoạt động. | Cần ghi nhận kết quả chạy chi tiết |
| TC-RES-01 | Survey result/export | Admin/Lecturer xem kết quả survey. | Hiển thị thống kê đúng phạm vi quyền, export báo cáo khi có dữ liệu. | Cần ghi nhận kết quả chạy chi tiết |
| TC-AI-01 | AI summary | Survey có comment text và cấu hình AI hợp lệ. | Generate hoặc xem AI summary theo quyền. | Cần ghi nhận kết quả chạy chi tiết |

Trong mã nguồn backend, các test hiện có bao phủ nhiều lớp: `AuthUseCaseServiceTest`, `SubmitSurveyServiceTest`, `AdminStudentApprovalServiceTest`, `AdminSurveyManagementServiceTest`, `GetSurveyResultServiceTest`, `StudentFeedbackServiceTest`, `GetStudentNotificationsServiceTest`, `SurveyAiSummaryServiceTest`, `SurveyReportExportServiceTest`, các persistence adapter test và controller test như `SurveyControllerTest`, `SurveyResultControllerTest`.
...
![API Testing Evidence - Swagger](assets/screenshots/security/api-testing.png)

> [Hình 9.1: Kiểm thử/tài liệu API bằng Swagger/OpenAPI, dùng cho các nhóm public, authenticated, admin và lecturer]
...
![GitHub Actions Pipeline Passed](assets/screenshots/deployment/github-actions-passed.png)

> [Hình 9.2: Screenshot GitHub Actions pipeline passed, thể hiện workflow CI hoàn thành thành công với các job chính]

Frontend hiện có lint/build trong CI nhưng chưa có framework unit test hoặc e2e test riêng như Vitest, Jest, Cypress hoặc Playwright. Đây là điểm cần bổ sung nếu muốn tăng mức tự động hóa kiểm thử giao diện.
...
![Portainer/Docker Stack Running](assets/screenshots/deployment/portainer-stack.png)

> [Hình 9.3: Portainer/Docker Stack Running, thể hiện backend/frontend container đang chạy]

![Prometheus Targets Up](assets/screenshots/monitoring/prometheus-targets-up.png)

> [Hình 9.4: Prometheus Targets Up, thể hiện target monitoring đang scrape thành công]

![Grafana Dashboard](assets/screenshots/monitoring/grafana-node-exporter-dashboard.png)

> [Hình 9.5: Grafana Dashboard, thể hiện dashboard CPU/RAM/Disk/Network/Uptime]
...
Các điểm nhóm cần tiếp tục hoàn thiện:
...
- Cần bổ sung kiểm thử tự động sâu hơn cho frontend, API regression và performance/load test khi hệ thống mở rộng.
...
Bảng sau tổng hợp các module chức năng đã được triển khai và các hình ảnh/bằng chứng minh họa trong báo cáo. Một số chức năng có thể dùng chung ảnh demo theo nhóm màn hình, ví dụ dashboard, survey management, feedback, notification hoặc AI summary.

| Module | Kết quả đạt được | Vai trò sử dụng | Bằng chứng minh họa |
|---|---|---|---|
...
![Login Page](assets/screenshots/ui/demo-login.png)

> [Hình 10.1: Login Page, thể hiện form đăng nhập và trạng thái xác thực]

![Student Dashboard](assets/screenshots/ui/demo-student-dashboard.png)

> [Hình 10.2: Student Dashboard, thể hiện góc nhìn của sinh viên sau khi đăng nhập]

![Admin Dashboard](assets/screenshots/ui/demo-admin-dashboard.png)

> [Hình 10.3: Admin Dashboard, thể hiện tổng quan quản trị hệ thống]

Nhóm ảnh quản trị khảo sát cần thể hiện cách Admin tạo, quản lý và phát hành survey.

![Survey Management](assets/screenshots/ui/demo-survey-management.png)

> [Hình 10.4: Survey Management, thể hiện danh sách khảo sát và trạng thái lifecycle]

![Create Survey](assets/screenshots/ui/demo-create-survey.png)

> [Hình 10.5: Create Survey, thể hiện form tạo/cập nhật khảo sát, câu hỏi và phạm vi người nhận]

Nhóm ảnh sinh viên tham gia khảo sát và kết quả khảo sát cần thể hiện luồng từ trả lời đến thống kê.

![Student Survey Detail](assets/screenshots/ui/demo-student-survey-detail.png)

> [Hình 10.6: Student Survey Detail, thể hiện nội dung survey và form trả lời]

![Survey Result Detail](assets/screenshots/ui/demo-survey-result-detail.png)

> [Hình 10.7: Survey Result Detail, thể hiện metrics, rating breakdown và comments]

Nhóm ảnh feedback, notification và AI cần thể hiện các tính năng bổ sung ngoài luồng khảo sát cơ bản.

![Feedback Management](assets/screenshots/ui/demo-feedback-management.png)

> [Hình 10.8: Feedback Management, thể hiện staff xem và phản hồi feedback]

![Notification Center](assets/screenshots/ui/demo-notification-center.png)

> [Hình 10.9: Notification Center, thể hiện notification list, unread/read state]

![AI Summary](assets/screenshots/ui/demo-ai-summary.png)

> [Hình 10.10: AI Summary, thể hiện bản tóm tắt phản hồi text và trạng thái generate]

### 10.3 Kết quả triển khai ứng dụng

Hệ thống đã được chuẩn bị artifact triển khai bằng Docker. Backend và frontend có Dockerfile multi-stage, CI build image và push lên GHCR khi có push vào nhánh được cấu hình. Cấu hình runtime được truyền qua environment hoặc `stack.env` trong Docker Compose.

![Docker/Portainer Stack](assets/screenshots/deployment/demo-portainer-stack.png)

> [Hình 10.11: Docker/Portainer Stack, thể hiện backend/frontend container đang chạy]

![GitHub Actions Quality Gate Demo](assets/screenshots/deployment/demo-github-actions-quality.png)

> [Hình 10.12: GitHub Actions Quality Gate, thể hiện backend test, frontend lint/build và Trivy filesystem scan passed]

![GitHub Actions Docker Job Demo](assets/screenshots/deployment/demo-github-actions-docker.png)

> [Hình 10.13: GitHub Actions Docker job, thể hiện Docker image build, Trivy image scan và push image lên GHCR]

![GHCR Backend Image](assets/screenshots/deployment/demo-ghcr-images-backend.png)

> [Hình 10.14: GHCR backend image package, thể hiện image tags đã được push lên GitHub Container Registry]

![GHCR Frontend Image](assets/screenshots/deployment/demo-ghcr-images-frontend.png)

> [Hình 10.15: GHCR frontend image package, thể hiện image tags đã được push lên GitHub Container Registry]

![Nginx Proxy Public Domain](assets/screenshots/deployment/demo-nginx-proxy.png)
...
Trong phạm vi mã nguồn, Docker Compose định nghĩa frontend và backend image từ GHCR. Việc deploy runtime được quản lý ở tầng vận hành, có thể thông qua Docker/Portainer hoặc compose stack riêng.
...
![Proxmox Overview](assets/screenshots/infrastructure/demo-proxmox.png)

> [Hình 10.17: Proxmox Overview, thể hiện node/VM/container liên quan hệ thống]

![Headscale Nodes](assets/screenshots/infrastructure/demo-headscale-nodes.png)

> [Hình 10.18: Headscale Nodes, thể hiện Tailnet nodes và tags]

![CrowdSec Decisions](assets/screenshots/security/demo-crowdsec-decisions.png)

> [Hình 10.19: CrowdSec Decisions, thể hiện quyết định ban IP hoặc phát hiện probing]

![Public Ports Check](assets/screenshots/infrastructure/demo-public-ports.png)
...
![Grafana Node Exporter Dashboard](assets/screenshots/monitoring/grafana-node-exporter-dashboard.png)

> [Hình 10.21: Grafana Node Exporter Dashboard, thể hiện CPU/RAM/Disk/Network/Uptime]

![Prometheus Targets Demo](assets/screenshots/monitoring/prometheus-targets-up.png)

> [Hình 10.22: Prometheus Targets demo, thể hiện target list trong phần demo monitoring và trạng thái `UP`]

![Loki Panel](assets/screenshots/monitoring/loki-logs-panel.png)

> [Hình 10.23: Loki Panel demo, thể hiện log observation trong Grafana]

![Telegram Alert](assets/screenshots/monitoring/demo-telegram-alert.png)

> [Hình 10.24: Telegram Alert, thể hiện contact point hoặc alert notification trong monitoring stack]
...
Mã nguồn dự án đã có Dockerfile cho backend/frontend, GitHub Actions pipeline, backend test, frontend lint/build, Trivy filesystem scan, Docker image build, Trivy image scan và push image lên GHCR. Đây là nền tảng DevSecOps quan trọng vì artifact triển khai được tạo tự động và có security gate trước khi được push.
...
- Frontend chưa sử dụng form/schema validation chuyên dụng như Zod, Yup hoặc React Hook Form.
- Frontend chưa sử dụng React Query/SWR để quản lý server state, cache, retry và invalidation.
...
- `docker-compose.yml` trong dự án hiện chủ yếu đóng gói frontend/backend.
...
Nhìn tổng thể, dự án tạo được nền tảng kỹ thuật đủ rõ để thành viên mới có thể tiếp nhận, bảo trì lâu dài và mở rộng có kiểm soát. Các hạn chế vẫn tồn tại, nhưng được nhóm nhận diện cụ thể và đề xuất hướng phát triển thực tế, thay vì bỏ qua trong báo cáo.
...
Một điểm quan trọng của đề tài là DevSecOps được đưa vào từ sớm. Dự án có Docker containerization, GitHub Actions, backend test, frontend lint/build, Trivy filesystem scan, Docker image scan và image artifact trên GHCR. Trường hợp Trivy phát hiện lỗ hổng critical và nhóm nâng Spring Boot parent để xử lý cho thấy security gate có giá trị thực tế.
...
Về mặt học thuật, đề tài giúp vận dụng nhiều nội dung của quy trình phát triển phần mềm: phân tích yêu cầu, phân tích nghiệp vụ, thiết kế kiến trúc, thiết kế cơ sở dữ liệu, xây dựng frontend/backend, kiểm thử, CI/CD, bảo mật và vận hành. Các chương trong báo cáo phản ánh quá trình đi từ bài toán đến mô hình hệ thống, từ mã nguồn đến hạ tầng, từ chức năng đến khả năng bảo trì.

Tổng kết lại, hệ thống đã có nền tảng tốt để tiếp tục đưa vào sử dụng, bảo trì và mở rộng. Dù vẫn còn hạn chế về CD tự động, kiểm thử tự động phía frontend, application metrics, secret management và log pipeline, kiến trúc hiện tại đủ rõ để nhóm tiếp tục phát triển theo hướng sẵn sàng hơn cho môi trường triển khai thực tế.

---

# TÀI LIỆU THAM KHẢO

Danh mục dưới đây là nhóm tài liệu tham khảo cần chuẩn hóa lại theo định dạng trích dẫn của học phần trước khi nộp bản cuối.
...
Nếu cần nộp kèm API contract chi tiết, phụ lục này có thể được mở rộng thành bảng endpoint đầy đủ gồm method, path, quyền truy cập, request và response mẫu.
...
Phụ lục này liệt kê các nhóm biến môi trường chính. Khi bàn giao triển khai, nhóm cần bổ sung bảng chi tiết gồm tên biến, ý nghĩa, ví dụ giá trị và mức độ nhạy cảm.

Các nhóm biến môi trường cần trình bày:

- Database connection.
- PostgreSQL connection cho `ai-translation-wrapper` translation cache.
...
- Ollama base URL/model, worker timeout, context size, max content length và GPU advisory lock key.
...
## Phụ lục D. AI Translation Worker ngoài repository chính

Repository worker:

```text
E:\Lap\TTCS\ai-translation-wrapper
```

Vai trò của repository này là triển khai service FastAPI xử lý dịch song ngữ bất đồng bộ cho `student-feedback-system`. Service không nhận request trực tiếp từ frontend; nó chạy RabbitMQ consumer nền, lấy task dịch từ `feedback.translate.queue`, gọi Ollama và gửi kết quả về `translation.reply.queue` để backend Spring Boot cập nhật dữ liệu.

Các thành phần chính:

- `main.py`: tạo FastAPI app, khởi tạo PostgreSQL session, AI service và RabbitMQ consumer trong lifecycle của ứng dụng; cung cấp endpoint `GET /health`.
- `app/core/config.py`: cấu hình qua environment variables, gồm PostgreSQL DSN, RabbitMQ topology, Ollama base URL/model, timeout, context size và giới hạn độ dài content.
- `app/worker/consumer.py`: khai báo queue/exchange, validate message, xử lý retry/DLQ, kiểm tra cache và publish reply.
- `app/services/ai_service.py`: gọi Ollama `qwen2:7b` bằng async client, yêu cầu output JSON song ngữ và validate format phản hồi.
- `app/db/session.py`: quản lý connection pool PostgreSQL, auto-migration bảng `translation_cache`, cache lookup/save và advisory lock cho GPU.
- `Dockerfile`, `docker-compose.yml`, `stack.env.example`: đóng gói và cấu hình runtime worker.
- `.github/workflows/docker-image.yml`: build/push Docker image lên GHCR.

RabbitMQ topology của worker:
...
```json
{
  "entity_id": 123,
  "entity_type": "SURVEY_RESPONSE",
  "translated_content_vi": "Nội dung tiếng Việt",
  "translated_content_en": "English content",
  "source_lang": "vi",
  "model_info": "qwen2:7b",
...
| Nhóm | Biến chính |
|---|---|
| PostgreSQL | `POSTGRES_DSN`, `POSTGRES_MIN_SIZE`, `POSTGRES_MAX_SIZE` |
| RabbitMQ | `RABBITMQ_URL`, `RABBITMQ_EXCHANGE`, `RABBITMQ_QUEUE`, `RABBITMQ_ROUTING_KEY`, `RABBITMQ_REPLY_EXCHANGE`, `RABBITMQ_REPLY_QUEUE`, `RABBITMQ_REPLY_ROUTING_KEY`, `RABBITMQ_DLQ`, `RABBITMQ_MAX_RETRIES` |
| Ollama | `OLLAMA_BASE_URL`, `OLLAMA_MODEL`, `OLLAMA_REQUEST_TIMEOUT_SECONDS`, `OLLAMA_NUM_CTX` |
| Worker limits | `MAX_CONTENT_CHARS`, `GPU_ADVISORY_LOCK_KEY` |

Thiết kế này bổ sung rõ phần microservice cho hệ thống: backend Spring Boot vẫn là ứng dụng nghiệp vụ chính, còn worker FastAPI chịu trách nhiệm xử lý tác vụ AI dịch nặng, có cache riêng bằng PostgreSQL và giao tiếp bất đồng bộ qua RabbitMQ.
...
Các ảnh/bằng chứng triển khai thực tế đã được bổ sung ở các chương Deployment, Network/Infrastructure, Monitoring và Security. Phụ lục này tóm tắt lại các thông tin hạ tầng chính đã được sử dụng trong báo cáo.

Các thông tin hạ tầng chính được sử dụng trong báo cáo:
